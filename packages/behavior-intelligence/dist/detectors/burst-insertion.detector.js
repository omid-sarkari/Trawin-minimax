    ...
    for (const event of editorEvents) {
        if (event.eventType === 'insert' || event.eventType === 'insertText') {
            ...
            if (payload && typeof payload.text === 'string')
                ...
                charCount = payload.lines.reduce((sum, line) => sum + (typeof line === 'string' ? line.length : 0), 0);
            }
            if (charCount > 0)
                insertions.push({ timestamp: new Date(event.createdAt).getTime(), charCount });
        }
    }
    return insertions.sort((a, b) => a.timestamp - b.timestamp);
}
function calculateWindowCPS(window) {
    if (window.length < 2)
        return 0;
...
const burstInsertionDetector = (input) => {
    const { editorEvents } = input;
    const insertions = extractInsertionEvents(editorEvents);
    if (insertions.length < 2)
        return { detectionType: 'burst_insertion', probability: 0, result: { totalInsertions: insertions.length } };
    const windows = [];
    let currentWindow = [];
    for (const insertion of insertions) {
        if (currentWindow.length === 0)
            currentWindow.push(insertion);
        else {
            const lastTimestamp = currentWindow[currentWindow.length - 1].timestamp;
            if (insertion.timestamp - lastTimestamp <= DEFAULT_CONFIG.windowSizeMs)
                currentWindow.push(insertion);
            else {
                if (currentWindow.length >= 1)
                    windows.push([...currentWindow]);
                currentWindow = [insertion];
            }
        }
    }
    if (currentWindow.length >= 1)
...
exports.burstInsertionDetector = burstInsertionDetector;
exports.default = exports.burstInsertionDetector;
